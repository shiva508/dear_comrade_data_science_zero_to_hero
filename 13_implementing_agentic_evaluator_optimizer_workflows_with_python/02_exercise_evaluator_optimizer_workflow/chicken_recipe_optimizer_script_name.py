...
class RecipeCreatorAgent:
    ...
    def create_recipe(self, recipe_request, feedback:None):
        ...
        system_prompt = "You are a culinary expert creating a delicious and easy-to-follow recipe."
        ...
        return call_openai(system_message, user_prompt)
...
def optimize_recipe(recipe_request):
    ...
    nutrition_agent = NutritionEvaluatorAgent()
    ...
    for attempt in range(MAX_RETRIES):
        attempts += 1
        print(f"\n--- Attempt #{attempts} ---")
        recipe= recipe_agent.create_recipe(recipe_request, feedback)
        evaluation = nutrition_agent.evaluate(recipe_request, recipe)
        print(f"\n📋 Evaluation Result:")
